...
function init() {
    ...
    let assumeBoard2 = []; //[focus] - later add it up to assumeBoard too
    function addeffect (x, y) {
        let lst = [-1.5, -1, -0.5, 0, 0.5, 1, 1.5 ]
        let slst = [-2, -1, 0, 1, 2]
        for (let i = 0; i < 15; i++){
            let rand = Math.floor(Math.random() * 7);
            let rand2 = Math.floor(Math.random() * 7)//last one dont count
            let rand3 = Math.floor(Math.random() * 5);
            let rand4 = Math.floor(Math.random() * 5)//last one dont count
            assumeBoard.push([lst[rand], lst[rand2], 30, x, y, [165, 76, 129]])
            assumeBoard2.push([slst[rand3], slst[rand4], 15, x, y, [0,0,0]])//[219, 165, 198]
    }
    }

    function showeffects(){
        console.log(assumeBoard2)
        ...
        for (let a of assumeBoard2){
            ...
            rect(Math.round(a[3]), Math.round(a[4]), Math.round(a[2]), Math.round(a[2]))
            if (a[2] < 1){
                let index = assumeBoard2.indexOf(a);
                assumeBoard2.splice(index, 1); 
            }
        }
    }
            ...
            for (let j = 0; j < rows; j++) {
                if (currentcolors == 1){
                    if (currentBoard[i][j] == 1 && nextBoard[i][j] == 1) {
                        ...
                        for (let b of bulletslst['mouseshoot']){ //[back here]
                            ...
                            if (b[1] < i * 20 + 20 &&
                                ...
                                // console.log(i, j, currentBoard[i][j])
                                // stroke(strokeColor);
...
let shootingbool = {'mouseshoot':false, 'entertrue':false} 
let bulletslst = {'mouseshoot':[]} // {bulletslst['mouseshoot'].push...=[]}
let shootspeed = 5;
