...
function init() {
    ...
    function addeffect (x, y) {
        let lst = [-3, -2, -1, 0, 1, 2, 3]
        let slst = [-2, -1, 0, 1, 2]
        for (let i = 0; i < 15; i++){
            let rand = Math.floor(Math.random() * 6);
            let rand2 = Math.floor(Math.random() * 6)//last one dont count
...
function showeffets(){
    for (let a of assumeBoard){
        ...
        fill(5[0], 5[1], 5[2])
        a[2] -= 0.1
        rect(a[3], a[4], Math.round(a[2]), Math.round(a[2]))
    }
...
function showbullet(){
    for (let b of bulletslst['mouseshoot']){
        ...
        if (b[1] < -20 || b[1]  > 1480 || b[2] < -20 || b[2] > 720){
            let index = bulletslst['mouseshoot'].indexOf(b);
            bulletslst['mouseshoot'].splice(index, 1)
        }
    }
