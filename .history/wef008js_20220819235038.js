...
for (let p of plst){
    p.addEventListener('click', function(){
        // selecting = true;
        if (p.innerHTML == 'Hammerhead'){
            let width = Math.round(columns / 2 * unitLength - (patterns['fir'][1] * unitLength)); //[1] is max length of each line
            let height = Math.round(rowss / 2 * unitLength - (patterns['fir'][2] * unitLength)); //[2] is size of lst
