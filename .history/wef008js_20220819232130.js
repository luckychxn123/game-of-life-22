...
for (let p of plst){
    p.addEventListener('click', function(){
        ...
        if (p.innerHTML == 'Hammerhead'){
            ...
            let heightt = Math.round(columns / 2 * unitLength - (patterns['fir'][2] * unitLength)); //[2] is size of lst
                for (let p of patterns['fir'][0]){
                    for (let innerp of p){
                        if (innerp == 'O'){
                            currentBoard[Math.round(width / unitLength)][Math.round(heightt / unitLength)] = 1;}
                                width += unitLength;
                        }

                    
                    heightt += unitLength;
                    widthh = Math.round(rows / 2 * unitLength - (patterns['fir'][1] * unitLength))
                    // problem solved: didnt set width again
            }
    } else if (p.innerHTML == 'Orthogonal ship'){
        let widthh = Math.round(rows / 2 * unitLength - (patterns['sec'][1] * unitLength)); //[1] is max length of each line
        let heightt = Math.round(columns / 2 * unitLength - (patterns['sec'][2] * unitLength)); //[2] is size of lst
        for (let p of patterns['sec'][0]){
            for (let innerp of p){
                if (innerp == 'O'){
                    currentBoard[Math.round(widthh / unitLength)][Math.round(heightt / unitLength)] = 1;
                }
                widthh += unitLength;
            }
            heightt += unitLength;
            widthh = Math.round(rows / 2 * unitLength - (patterns['sec'][1] * unitLength))
            // problem solved: didnt set width again
        }
    } else if (p.innerHTML == 'Bear'){
        let widthh = Math.round(rows / 2 * unitLength - (patterns['thir'][1] * unitLength)); //[1] is max length of each line
        let heightt = Math.round(columns / 2 * unitLength - (patterns['thir'][2] * unitLength)); //[2] is size of lst
        for (let p of patterns['thir'][0]){
            for (let innerp of p){
                if (innerp == 'O'){
                    currentBoard[Math.round(widthh / unitLength)][Math.round(heightt / unitLength)] = 1;
            ...
            heightt += unitLength;
            widthh = Math.round(rows / 2 * unitLength - (patterns['thir'][1] * unitLength))
            // problem solved: didnt set width again
        }
    }})
    }
...
function setup(){
    ...
    columns = floor(width  / unitLength);
    rows    = floor(height / unitLength);
    console.log(rows, 'rows', columns, 'c')
...
framerateslider.addEventListener("input",(e)=>{
    ...
    fr = e.target.value;
    console.log('froutput.innerHTML',froutput.innerHTML, 'fr', fr, 'e.target.value', e.target.value)
